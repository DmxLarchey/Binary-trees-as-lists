...
intros H0 H1.
split. (* Decouper le ET*)
reflexivity.
apply H1.

intros.
discriminate. (*Hypotheses absurdes 0 = S ?*)

intros.
discriminate. (*Hypotheses absurdes 0 = S ?*)

intros.
inversion H. (* On prend la condition necessaire à H*)
...
Section flat_map.
  ...
  Proof.
    (* induction sur l1*)
    induction l1.
    (*Par rapport a la definition de flat_map*)
    constructor.
    simpl.
    (* On peut reecrire à l'aide de l'hypothese d'induction *)
    rewrite IHl1.
    rewrite app_ass.
    (*Egalité des deux cotés*)
    reflexivity.
  Qed.
