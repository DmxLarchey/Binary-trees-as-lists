...
Proof.
  ...
  intros; injection H0.
  intros; subst.
  case (IH l2); auto; intros; subst; auto.
Qed.
...
Section flat_map.
  ...
  Proof.
    (* induction sur l1*)
    induction l1.
    (*constructor ou reflexivity marche car égalité defini dans construct de flat_map*)
    reflexivity.
    simpl.
    (*on peux reecrire la partie gauche grace à l'hypothese *)
    rewrite IHl1.
    rewrite app_ass.
    (* deux membre identique reflexivity ou auto *)
    reflexivity.
  Qed.
  ...
  Proof.
    ...
    auto.
    inversion H.
    replace (a :: l) with (nil ++ a :: l).
    case (app_cons_not_nil nil l a).
...
Section prefix. (* as an inductive predicate *)
  ...
  Proof.
    ...
    simpl; intros ; apply prefix_inv in H.
    destruct H as (Y & H); subst.
