...
Section flat_map.
  ...
  Proof.
    ...
    auto.
    rewrite IHl1.
    rewrite app_ass. (* enleve les parentheses à droite *)
    reflexivity.
  Qed.
  ...
  Proof.
    ...
    inversion H.
    symmetry.
  (* utiliser nil_cons pour prouver la contradiction *)
    Check length_nil.
...
Section prefix. (* as an inductive predicate *)
  ...
  Proof.
    ...
    intros;left;apply in_prefix_0.
    destruct r1 as [ | y r1 ].
    intros;right;apply in_prefix_0.
    intros.
    (* apply prefix_inv in H.
    destruct H.
    subst. *)
...
Proof.
  (*revert l2 m; induction l1 as [ | x l1 IH ]; intros l2 r1 r2.*)
  intro.
...
Proof.
  (* decide equality *)
  (*  This tactic solves a goal of the form forall x y : R, {x = y} + {~ x = y}, 
      where R is an inductive type such that its constructors do not take proofs or functions as arguments, 
      nor objects in dependent types. It solves goals of the form {x = y} + {~ x = y} as well.
  *)
...
Proof.
  (*
  generalize <<a1,a2>> as a.
  generalize <<b1,b2>> as b.
  intros.
  left.
  intuition.
  apply H.
...
(* Probleme de caractere, << t >> ne passe pas *) 
(*
...
(*
Depend de bt_bin_length qui est commenté
*)
Fact bt_bin_length_geq t : 1 <= length [[t]].
Proof.
(*
  rewrite bt_bin_length.
  induction t.
  constructor.
  replace (<< << t1, t2 >> >>) with (1 + << t1 >> + << t2 >>).
  omega.
